...
import edu.berkeley.cs162.TPCMaster;
...
public class EndToEndTests {
	...
	public void tearDown() throws Exception {
		...
		master.stop();
		// Wait for the reg server to actually stop
		Thread.sleep(1000);
		master = null;
	}
	...
	@Test(timeout = 15000)
	public void testDel() throws UnknownHostException, KVException {
	    System.out.println("INFO EndToEnd.testDel: Begin.");
		KVClient client = new KVClient(InetAddress.getLocalHost().getHostAddress(), 8888);

		try {
			client.del("foo");
			fail("Delete did not fail for non-existent key.");
		} catch (KVException e) {
		    // Expected
		}

		// Put our key in (these are just expected to work.
	    client.put("foo", "bar");
		...
		try {
		    client.get("foo");
		    fail("Get value for deleted key did not fail.");
		} catch (KVException e) {
		    // Expected
		}

		System.out.println("INFO EndToEnd.testDel: Finished.");
	}

	@Test
	public void simpleKillSlave() {
